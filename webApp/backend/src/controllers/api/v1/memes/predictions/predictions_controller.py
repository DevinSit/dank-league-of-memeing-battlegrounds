...
def get_predictions() -> Response:
    ...
    if "imageHashes" not in data:
        logger.warning("Image hashes are missing from payload.")
        return jsonify(abort(400))

    predictions = list(map(lambda image_hash: keras_prediction.get_prediction(image_hash), data["imageHashes"]))
